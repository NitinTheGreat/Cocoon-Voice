...
- **Routes/commands:** `POST /v1/sessions/{id}/impacts`, `POST /v1/sessions/{id}/presence`, `POST /v1/sessions/{id}/events/{event_id}/presentation` (both promoted from proposed; runtime names `ConsumerPresenceReport`, `PresentationReceipt` avoid the proposed names), command `sos.respond`, voice "I'm okay" / "I need help", `/state.sos`, `/state.presence`, `Announcement.presentations`, overview `sos`, feed `sos.changed`. Announcement type `sos_checkin`.
- **Rules:** eligible offer = `played` delivery or screen `presented` report of the check-in event; vibration, failed/unknown playback, presence and feed receipt never count. Offer wait / response window 60 s / 90 s (standard) or 4 s / 5 s (`accelerated_demo`, per process). Deadline wins at the exact instant. Help notifies at once; no answer after an offer → `sos_no_response`; no offer → `sos_unreachable` with `no_live_channel` or `offer_not_confirmed`; late okay → separate `sos_recovered`. One notification per (kind, episode). Candidates older than 300 s are `historical`; merged impacts never move deadlines.
- **Checks:** `tests/test_sos.py` 8 passed (voice offer + okay, no notification; screen offer, identical/conflicting/later reports, no-response notification once, late okay recovery; immediate help with duplicate command; no-channel vs unconfirmed-offer; blocked_no_policy and blocked_no_recipient; duplicate/merge/historical/below-threshold/future/supervisor-denied candidates; exact-deadline race and bare "yes" clarification; a real `python -m cocoon_agent` process killed while a check-in was pending and restarted: the worker marked it unreachable and created exactly one notification without new input). Full suite: 399 passed, 1 failed before the capability-table update (fixed), 2 skipped.
- **Not verified:** any wearable or fall detection (all candidates simulated), phone playback/screen rendering, supervisor UI, real-site emergency policy.
...
- **Consent:** `GET/POST /v1/operators/{operator_id}/consents` (promoted from proposed; runtime `OperatorConsentState` etc.). Only the operator's own token writes (service/supervisor 403, other operator 404); notice version checked; `expected_version` stops an old offline grant overriding a newer revocation; an identical retry returns `applied: false` and is never re-applied; sharing needs processing and is revoked with it. Revocation deletes raw samples, withdraws a vitals-based episode (`consent_revoked`, not recovery) and writes a `risk.changed` feed reference. Voice reads the summary only.
- **Wellbeing:** `POST /v1/sessions/{id}/wellbeing/samples` and `GET /v1/sessions/{id}/wellbeing`, `/state.wellbeing`, `break.start`/`break.end` (tap and voice), voice "why did you suggest a break?". One deterministic policy (`policies/wellbeing_v1.json`): NWS heat index (published formula and bands), break due after 120 min, mean HR over 5 min with >= 3 samples (demo assumptions); hysteresis, unknown on missing data, one episode per operator announced once (no values in speech). Supervisor projection `WellbeingRiskView` (category, availability, freshness) recomputed from current consent on each read.
- **Contracts:** both regenerated; the proposed builder now refuses a contract-package model sharing a runtime component name (`check_schema_collisions`).
- **Checks:** `tests/test_wellbeing.py` 7 passed (heat-index branches; consent ownership/version/idempotency/cascade; no retention, echo, dump or log of a sentinel value without consent; 422 without echo; advice open/duplicate/explain/hold on missing data/late sample/clear; heat and break without vitals; explicit breaks leave tasks unchanged; revocation deletes samples, withdraws advice, hides risk; retention purge). Full suite after D1: 381 passed, 2 skipped (before the collision test was added).
- **Not verified:** any real wearable, WESAD (absent, DG-15), privacy review of the notices, live-model routing of the new voice intents.

## Batch C completion: combined HTTP demonstration and summary

- **Demo:** `python scripts/demo_required_features.py` (reuses `demo_operator.py`, `seed_demo.py` and the simulator scenarios; isolated `data/demo_c`; mock LLM; fixture weather; simulation clock pinned to the site-local service date). Observed on 2026-09-24 over real HTTP: briefing with conditions and estimate -> assigned tasks with conditions/estimates -> conditions check needing acknowledgement -> "start anyway" (check + start estimate saved) -> proximity warning, danger escalation (announced), observed clear -> "why" from saved evidence incl. the escalation -> three belt episodes -> pending `repeated_violations` review + L1 assignment -> coaching prompt once parked -> lesson metadata and video download (SHA-256 matched; not playback) -> five spoken steps -> quiz ("yes" rejected, 3/3 passed, lesson complete) -> progress -> sudden stop, slope, worsening conditions (block) and fuel per cycle episodes -> incident with "ten minutes ago" + incident asked for severity -> task completed by tap (start check and estimate kept) -> estimator evaluation (MAE 7.6 = baseline). Re-running the same run ID replayed everything: identical spoken replies, 39 duplicate telemetry samples, the tap reported `duplicate: true`, same final state. B's `demo_operator.py` still completes (it now keeps weather off by default because it runs on the wall clock).
- **Fixes found by the demo:** repeat-count and fuel-trend episodes no longer hold back coaching prompts (only hazard warnings do); "why" now says when a warning rose to danger/block; coaching prompts state the lesson's intended length.
- **Final checks (once, after assembly):** full suite 374 passed, 2 skipped (dataset replay test: dataset absent; Windows symlink case); `export_openapi.py --check`, `export_proposed_contract.py --check`, `evaluate_estimator.py --check` and `make_lesson_video.py --verify` all clean.
- **Environment of these checks:** Linux, Python 3.11.15 (uv), no `Cocoon_Dataset_v1` (labelled stand-in catalog in the demo; synthetic test catalogs in tests), mock LLM only (no Vertex call in Batch C), one live Open-Meteo request.
- **Still open for Batch D and beyond:** supervisor decisions on pending reviews, consent/wellbeing, weather re-planning, SOS, offline sync, streaming; live-model classification of every new intent/field; source validation of every numeric limit; trainer review of lesson content; estimator calibration on `task_history` once the dataset is available; phone playback of the media; the unresolved hard-kill observation (MIGRATIONS.md).

## C4: core LMS (curriculum, media, quizzes, progress, levels, coaching)

- **Schema v12 (`core_lms`):** `lesson_versions` (pinned lesson copies incl. answer keys, server-side only), `lesson_progress`, `quiz_attempts` (one active per learner and lesson), `quiz_answers`, `learner_levels`, `level_history`; `training_assignments.completed_at/deferred_until/coaching_prompted_at`.
- **Content:** `content/curriculum_v1.json` (8 lessons, quizzes, scenario, media catalog, level criteria, retake and defer policies; original demo text, general themes of the cited Caterpillar safety article paraphrased, no numeric limit taken from it; `demo_authored_unreviewed`). Seeded idempotently at startup; a changed lesson under an existing version is refused.
- **Media:** `content/media/L1_seatbelt_demo_v1.mp4` (+ `.vtt`), an original captioned clip generated by `scripts/make_lesson_video.py` (reproducible, byte-identical re-runs observed). Served only by catalog ID inside `content/`, only while its bytes match the catalog SHA-256; fetching it never changes progress.
- **Flow (voice and taps share the same mutations and command log):** `lesson.start/next/pause/resume/defer`, `quiz.start`, `quiz.answer`. A lesson completes only after every step was presented and its assessment passed; a failed attempt keeps it `awaiting_assessment`; retakes are new attempts pinned to the version. Voice: "start the seatbelt lesson", "next", "pause", "continue my lesson", "later", "quiz me", "retake the quiz", answers by letter, ordinal or distinctive words (unclear -> asked again; a bare "yes" never answers), "what training do I need?", "how am I progressing?". Learner = the catalog operator of a verified session; progress resumes across restarts and new sessions and is invisible to other operators; legacy sessions have none.
- **API:** `GET /v1/sessions/{id}/lessons`, `GET /v1/sessions/{id}/lessons/{lesson_id}`, `GET /v1/content/{asset_id}` (promoted from proposed; the fuller `ContentAsset` stays the I12A target), `/file`, `/captions`; `/state.learning`; announcement type `coaching_prompt`; command kinds above.
- **Behaviour changes to existing tests:** "start the seatbelt training" now starts the lesson (assign stays "assign me ..."), the lesson catalogue has 8 entries, and the B selection replay now ends with one coaching prompt (the belt lesson offered once the warning cleared while idle).
- **Checks:** `tests/test_lms.py` 8 passed (no answer keys in public payloads; media metadata, SHA-256, ETag, captions, auth, no path traversal, ffprobe + full ffmpeg decode of the downloaded file; voice lesson -> fail -> retake -> pass with replay and "yes" rejection; taps with duplicate replay, stale step/question 409 and bad choice 422; pause/defer/resume across restart and a new session with operator isolation; levels with evidence and the branching scenario; coaching only when parked and warning-free, once, after deferral; legacy sessions). Full suite: 374 passed, 2 skipped.
- **Not verified:** playback on an actual phone, content review by a trainer, live-model mapping of spoken quiz answers.

## C3: explainable task-duration estimates and the five-row benchmark

- **Schema v11 (`task_estimates`):** saved estimates keyed by task + estimator config SHA-256 + input snapshot; `task_assignments.start_estimate_id` (estimate in force at the start) and `ground_condition` (fixture 1.1 extension, filled once by the seed).
- **Estimator (`cocoon_agent/estimation.py`, `estimation/estimator_v1.json`):** a pure function over a frozen configuration; no training at startup, no refit per request, no language model. Paths: `productivity_with_factors` (quantity / configured rate for the machine category, task type and unit), `provided_estimate_adjusted` (a planner's pre-task estimate, never an actual duration), `typical_duration_fallback`, `not_estimable`. Factors (only when the input is known): ground condition, dataset operator skill (from an optional catalog column, separate from LMS levels), machine age (optional catalog column), numeric weather at the scheduled start from the same weather source as C2 (rain rate, gusts, heat, visibility) or categorical weather. Each result lists method, base, per-factor minutes, missing inputs, input snapshot, config hash and `calibration_status: uncalibrated_configured_prior`; no confidence interval is claimed.
- **Use:** `AssignedTask.estimate` (saved on read, stable id per snapshot), `start_estimate` saved inside the start transaction (voice and tap; a replayed start keeps it; later weather never rewrites it), `elapsed_minutes` separate; the next-task speech, the shift briefing and "how long will this take?" use the saved estimate.
- **Benchmark (`scripts/evaluate_estimator.py`, rows in `estimation/provided_benchmark_v1.json`, report `estimation/benchmark_report_v1.json`):** baseline recomputed from the rows: |errors| 2, 7, 12, 2, 15 = 38 min, MAE 7.6, bias -6.0. Estimator (reduced path, no context factor available for these rows in this checkout): predictions equal the provided estimates, MAE 7.6, bias -6.0: **no improvement**. Audit: nothing fitted; the configuration author had seen the actuals, and the dataset generator cannot be audited here, so the result is labelled `illustrative_benchmark_not_independent`. No synthetic train/validation split was possible.
- **Blocked:** calibration on `task_history` needs `Cocoon_Dataset_v1` (not present on this machine); the source CSV's weather/skill/age columns for T001–T005 are likewise unavailable, so they are null in the tracked rows rather than guessed.
- **Checks:** `tests/test_estimation.py` 5 passed (factor path and determinism, missing-input fallbacks and labels, baseline arithmetic + committed report reproducible, saved/started estimate stability across weather change and replay, catalog skill/age parsing). The B next-task speech assertion now includes the estimate. Full suite: 366 passed, 2 skipped.

## C2b: remaining hazard and behaviour rules

- **Schema v10 (`hazard_rules`):** `alerts.subject_key` with the unique active index now per (session, rule, subject) (existing rows get '' so belt/idle keep one per rule); `alerts.level`, `last_seen_at`, `cleared_reason`; `alert_updates` (later level changes with their own evidence and optional announcement); `machine_state.rule_state_json` (fuel window, last evaluated motion sample, per-rule coverage); `approval_requests` rebuilt to add kind `repeated_violations`, an optional `incident_id`, `alert_id` and `details_json` (rows copied unchanged).
- **Rules (`cocoon_agent/hazards.py`, `policies/hazard_rules_v1.json` `demo-hazards-2026-09-24.1`, all limits `synthetic_demo_assumption`):** proximity (per entity, warning -> danger in one episode, observed clear, `lost` then `expired_without_detection`, never "left"), sudden start/stop (instant events from dense timestamped speed samples; gaps, non-positive steps and too few samples are unknown; no double counting), steep slope (per-category pitch/roll limits; unconfigured truck/backhoe are `not_configured`; grade converted only when pitch is absent; hysteresis), fuel per load cycle (covered task window, gap exclusion, reset discard, zero cycles left to idle rules, configured baselines only), repeated violations (distinct episodes per family in 60 min observation time, overlaps merged; one pending review and one coaching assignment per trigger). They run inside the observation transaction after the belt/idle rules, with no model call; the working-conditions episode now also records level changes (acknowledge -> block is an announced update).
- **API (additive):** telemetry `readings` gain `proximity`, `motion`, `pitch_deg`, `roll_deg`, `grade_pct`, `fuel_meter_l`, `load_cycles_total`; `TelemetryResult.alerts_updated`; `Alert.subject_key/level/cleared_reason/updates`; announcement type `alert_escalated`; `/state.rule_coverage`. Idempotency digests omit the new optional fields while unset, so requests saved before the upgrade still replay (payload fields added in C1/C2 are treated the same way).
- **Simulator:** `scripts/simulate_machine.py --scenario proximity_approach|proximity_lost|sudden_stop|slope|fuel_high|fuel_normal|fuel_reset|repeat_belt|normal_operation` (synthetic, stable IDs).
- **Checks:** `tests/test_hazards.py` 10 passed (normal operation opens nothing; proximity escalation/de-escalation/observed clear/lost/expiry; sudden stop with gap, reorder and replay; slope with hysteresis, not_applicable and not_configured, grade conversion; fuel high/normal/reset/zero cycles/gaps/no baseline/no task; repeat trigger once with pending review and one lesson, duplicates not counted; malformed inputs 422 and late samples ignored). The C2a worsening test now expects the announced acknowledge -> block update. Full suite: 361 passed, 2 skipped.
- **Not validated:** every distance, acceleration, tilt, fuel baseline and repeat threshold is a demo assumption; no real detector, IMU or fuel meter; "why" for the new families was checked in mock mode only.

## C2a: site weather and pre-task working-condition checks

- **Schema v9 (`site_conditions`):** `sites.latitude/longitude/location_basis` (filled once by the seed from the fixture's labelled synthetic location), `weather_records` (weather exactly as used), `condition_checks` (saved checks with findings), `task_assignments.start_check_id`, `alerts.details_json` (family-specific episode evidence).
- **Weather (`cocoon_agent/weather.py`):** `COCOON_WEATHER_MODE=fixture|live|off`, independent of the LLM mode. Fixture periods are matched to the session's data clock (newest applied observation, else wall clock) in the site's time zone. Live: Open-Meteo `current` (15-minute) + 6 `hourly` records for the trusted coordinates, units checked against the response and normalised (unknown unit = null, `quality: partial`), precipitation kept with its summing window and as a rate, no issue time (the provider gives none). A per-site cache is refreshed by a background task (bounded interval, 5 s timeout); every lookup is cache-only and returns `fresh`/`stale`/`unavailable`/`misaligned` (a replay far from now cannot use present-day weather). A failure never substitutes synthetic weather.
- **Checks (`cocoon_agent/conditions.py`, `policies/working_conditions_v1.json` `demo-conditions-2026-09-24.1`):** per-variable advisory/acknowledge/block levels with task overrides; outdoor zones only; missing weather is `unknown` and treated as an advisory. Voice and tap `task.start` share the gate inside the start transaction; the saved check is returned and kept on the task. Voice: an acknowledge-level finding asks once (`task_start_ack`), "start anyway" (or a lone "yes") confirms, "wait" cancels. Tap: `payload.acknowledge_conditions`; a block is 409 whatever the acknowledgement. An in-progress outdoor task whose conditions worsen past its start level and the policy minimum opens a `working_conditions` episode (announced, explainable, evidence saved once, cleared when conditions return). "What are the conditions?" answers from the same check; weather is no longer an unsupported capability.
- **Tests:** existing tests now run with `COCOON_WEATHER_MODE=off` so they do not depend on the hour; two exact-dict comparisons gained the new null keys, the briefing wording check and the unsupported-capability test were updated because the behaviour changed as C2 requires. `tests/test_conditions.py` 18 passed (policy table incl. gaps and task overrides, fixture alignment, Open-Meteo parsing with km/h conversion and an unknown unit, stale/misaligned/failure coverage, voice acknowledgement and replay, tap acknowledgement and block, worsening episode + "why", missing weather, a hanging provider not delaying a belt warning). Full suite once: 351 passed, 2 skipped (dataset replay test and the Windows symlink case).
- **Live provider:** `python scripts/weather_smoke.py` on 2026-09-24 ~23:45Z received and validated 7 records (grid cell 12.970123, 79.11818; current gust 11.9 m/s = advisory). This proves the adapter against the real API once; it is model output for a grid cell, not a machine sensor.
- **Not validated:** every numeric limit is a demo assumption (no OEM/site source); the site location is synthetic.
