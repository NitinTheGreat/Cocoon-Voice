...
- Initial testing uses LiveKit Playground or its current Agent Console equivalent. Demo capabilities: ask for tasks, report an incident, request a lesson, hear an unsolicited simulated warning, and ask why it happened.
...
- Pipeline: STT produces a completed utterance; an HTTP call runs LangGraph; returned `speech` goes to TTS. No realtime speech-to-speech API, local reasoning agent, or second reasoning LLM here.
...
- Select one working STT/TTS setup, preferably LiveKit Inference where available. Make valid model/voice identifiers configurable; verify them for the project and SDK version. Avoid a general provider framework.
...
- No MQTT, Redis, Kafka, Celery, vector database, Kubernetes, custom WebRTC signaling, full LMS, wearable ML, or Android UI in this foundation. Use short nonstreaming HTTP responses first.
- Noise mitigation, disconnected operation, proximity sensing, heart rate/temperature features, and predictive models are future ideas. Record them as future scope, not delivered capabilities. This prototype does not control equipment.
...
| --- | --- | --- |
| Architecture | DESIGN AGREED | LiveKit worker; STT + remote HTTP LangGraph + TTS; one operator per room. |
| Existing code, SDK, and providers | UNVERIFIED | Inspect checkout and installed versions; reconcile existing work. |
| HTTP adapter, contract, and mock server | UNVERIFIED | Check canonical schemas and actual real-backend compatibility. |
| Announcement scheduling and delivery | UNVERIFIED | Check cursor, interruption, reconnect, and playback evidence. |
| Commands, worker dispatch, and live audio | UNVERIFIED | Record offline, real HTTP, and Playground results separately. |

### Active work

| Task | Owner / branch | Status | Files / contract impact | Next checkpoint |
| --- | --- | --- | --- | --- |
| Repository reconciliation | Unassigned | TODO | Inspect voice worker and shared contract; no implementation claims yet. | Record actual state and choose the next incomplete flow. |
...
- **Blocking information:** live credentials, actual SDK/provider configuration, and exact problem-statement requirements are not established here; inspect available materials without printing secrets.
